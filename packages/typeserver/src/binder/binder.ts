...
import {
    ...
    getEnclosingLambda,
    getEvaluationNodeForAssignmentExpression,
    getExecutionScopeNode,
...
export class Binder extends ParseTreeWalker {
    ...
    override visitFunction(node: FunctionNode): boolean {
        ...
        const symbol = this._bindNameToScope(this._currentScope, node.d.name);
        const containingClassNode = getEnclosingClass(node, /* stopAtFunction */ true);
    ...
    override visitCall(node: CallNode): boolean {
        this._disableTrueFalseTargets(() => {
            ...
            const sortedArgs = getArgsByRuntimeOrder(node);
        ...
        if (!isNodeContainedWithinNodeType(node, ParseNodeType.Decorator)) {
    ...
    override visitAssignmentExpression(node: AssignmentExpressionNode) {
        ...
        const evaluationNode = getEvaluationNodeForAssignmentExpression(node);
        if (!evaluationNode) {
    ...
    override visitTypeAnnotation(node: TypeAnnotationNode): boolean {
        ...
        const bindVariableBeforeAnnotationEvaluation =
            ...
            getEnclosingClass(node, /* stopAtFunction */ true) !== undefined;
    ...
    override visitAwait(node: AwaitNode) {
        ...
        const execScopeNode = getExecutionScopeNode(node);
    ...
    override visitImportFrom(node: ImportFromNode): boolean {
        ...
        if (node.d.isWildcardImport) {
            if (getEnclosingClass(node) || getEnclosingFunction(node)) {
    ...
    private _addPatternCaptureTarget(target: NameNode) {
        ...
        if (this._currentMatchSubjExpr) {
            if (
                isMatchingExpression(target, this._currentMatchSubjExpr) ||
                isPartialMatchingExpression(target, this._currentMatchSubjExpr)
            ) {
                this._currentMatchSubjExpr = undefined;
            }
        }
        ...
        switch (target.nodeType) {
            case ParseNodeType.Name: {
                ...
                if (symbolWithScope && symbolWithScope.symbol) {
                    ...
                    if (finalInfo.isFinal) {
                        const containingClass = getEnclosingClassOrFunction(target);
    ...
    private _getVariableDocString(node: ExpressionNode): string | undefined {
        const docNode = getVariableDocStringNode(node);
        if (!docNode) {
            return undefined;
        }
    ...
    private _getMemberAccessInfo(node: MemberAccessNode): MemberAccessInfo | undefined {
        ...
        const methodNode = getEnclosingFunction(node);
        if (!methodNode) {
            return undefined;
        }

        const classNode = getEnclosingClass(methodNode, /* stopAtFunction */ true);
        if (!classNode) {
            return undefined;
        }
