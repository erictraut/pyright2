 ...
 * Author: Eric Traut
 *
 * A service that is able to perform type analysis a collection of
 * Python files. It is able to track changes to the files and updates
 * analysis results (including any diagnostics) as needed.
 */
...
export class TypeService {
    ...
    clone(instanceName: string, fileSystem?: FileSystem): TypeService {
        ...
        service._program.setTrackedFiles(userFiles);
        service._program.markAllFilesDirty(true);

        // Make sure we keep editor content (open file) which could be different than one in the file system.
        for (const fileInfo of this._program.getOpened()) {
    ...
    runEditMode(callback: (e: IEditableProgram) => void, token: CancellationToken) {
        ...
        try {
            ...
            edits = this._program.exitEditMode();
        }
    ...
    setCompletionCallback(callback: AnalysisCompleteCallback | undefined): void {
        ...
        this._program.setCompletionCallback(callback);
    }
    ...
    hasSourceFile(uri: Uri): boolean {
        return this._program.hasSourceFile(uri);
    }
    ...
    getChainedUri(uri: Uri): Uri | undefined {
        return this._program.getChainedUri(uri);
    }

    updateChainedUri(uri: Uri, chainedFileUri: Uri | undefined) {
        this._program.updateChainedUri(uri, chainedFileUri);
        this.scheduleReanalysis(/* requireTrackedFileUpdate */ false);
    }
    ...
    setFileClosed(uri: Uri, isTracked?: boolean) {
        this._program.setFileClosed(uri, isTracked);
        this.scheduleReanalysis(/* requireTrackedFileUpdate */ false);
    }
    ...
    analyzeFileAndGetDiagnostics(fileUri: Uri, token: CancellationToken): Diagnostic[] {
    ...
    invalidateAndForceReanalysis() {
        ...
        this._program.invalidateAndForceReanalysis();
    }
    ...
    protected runAnalysis(token: CancellationToken) {
        ...
        if (!this.options.usingPullDiagnostics) {
            const moreToAnalyze = this._program.startAnalysis(token);
            if (moreToAnalyze) {
                this.scheduleReanalysis(/* requireTrackedFileUpdate */ false);
            }
        }
    ...
    private get _watchForLibraryChanges() {
        return !!this._commandLineOptions?.languageServerSettings.watchForLibraryChanges;
    }
    ...
    private _getConfigOptions(host: Host, commandLineOptions: CommandLineOptions): ConfigOptions {
        ...
        if (commandLineOptions.configFilePath) {
            ...
            if (!this.fs.existsSync(configFilePath)) {
                ...
                if (configFilePath.lastExtension.endsWith('.json') || configFilePath.lastExtension.endsWith('.toml')) {
                    ...
                    configFilePath = this._findConfigFile(this.fs, configFilePath);
                    if (!configFilePath) {
            ...
            configFilePath = this._findConfigFile(this.fs, projectRoot);
            ...
            if (!configFilePath && !commandLineOptions.fromLanguageServer) {
                configFilePath = this._findConfigFileHereOrUp(this.fs, projectRoot);
            }
        ...
        if (!configFilePath) {
            ...
            pyprojectFilePath = this._findPyprojectTomlFile(this.fs, projectRoot);

            if (!pyprojectFilePath && !commandLineOptions.fromLanguageServer) {
                pyprojectFilePath = this._findPyprojectTomlFileHereOrUp(this.fs, projectRoot);
            }
    ...
    private _updateTrackedFileList(markFilesDirtyUnconditionally: boolean) {
        ...
        if (this._typeStubTargetImportName) {
            ...
            const importResult = this._program.importResolver.resolveImport(Uri.empty(), execEnv, moduleDescriptor);
            ...
            this._program.setTrackedFiles(fileList);
            ...
            try {
                ...
                this._sourceFileWatcher = this.fs.createFileSystemWatcher(fileList, (event, path) => {
                    ...
                    if (eventInfo.isFile && eventInfo.event === 'change') {
                        this._program.markFilesDirty([uri], /* evenIfContentsAreSame */ false);
                        this.scheduleReanalysis(/* requireTrackedFileUpdate */ false);
                        return;
                    }
